...
u8 rcr_remainder_Y;   //重复计数余数部分
u8 is_rcr_finish_Y=1; //重复计数器是否设置完成
long rcr_integer_Y;	//重复计数整数部分
...
void Driver_Init_Y(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_14|GPIO_Pin_15;	//PC0.2 端口配置
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		//推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		//IO口速度为50MHz
	GPIO_Init(GPIOE, &GPIO_InitStructure);					//根据设定参数初始化GPIOC
	GPIO_SetBits(GPIOE,GPIO_Pin_14);						 	//PC0输出高 顺时针方向  DRIVER_DIR_Y
	GPIO_SetBits(GPIOE,GPIO_Pin_15);						//PC2输出低 使能输出  DRIVER_OE_Y
}
